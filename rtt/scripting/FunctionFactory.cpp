...
namespace RTT {
    using namespace detail;
        ...
        base::DataSourceBase::shared_ptr FunctionFactory::produceHandle() const {
            return new CmdCollectDataSource();
        }
        ...
        base::DataSourceBase::shared_ptr FunctionFactory::produceCollect(const std::vector<base::DataSourceBase::shared_ptr>& args, DataSource<bool>::shared_ptr blocking
            ...
            if (args.size() >= 1) {
                if ( dynamic_cast<CmdFunction* > (args[0].get()) != 0 ) {
                    // args[0] is the result of sending a script function.
                    // ==> produce a data source that retrieves the SendStatus
                    return new CmdCollectDataSource( static_cast<CmdFunction* > (args[0].get()) );
                } else if ( dynamic_cast<CmdCollectDataSource* > (args[0].get()) != 0 ) {
                    // args[0] is already a collect data source that was wrapped in a SendHandleAlias.
                    // This case is triggered from ExpressionParser::seendatacall() for sh.collectIfDone().
                    return args[0];
                } else {
